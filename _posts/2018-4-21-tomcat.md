...
- 根据上述代码，请求行和请求头的读取本质是调用 java NIO api SocketChannel 的 read() 方法，该方法为非阻塞方法。如果读不到数据就直接返回，继续由[以前文章](http://mp.weixin.qq.com/s?__biz=MzI0MDE3MjAzMg==&mid=2648393626&idx=1&sn=36cf6e3db8d31920a908afa8ce70bd38&chksm=f1310af5c64683e3b7875f73a1a46ee0e667ad7c4507b4ea74f2468c8645eefdb0c903f9702c&scene=21#wechat_redirect)介绍的 poller 线程监测是否有数据可读。
- 对于请求体的读取采用阻塞的方式，调用[以前文章](http://mp.weixin.qq.com/s?__biz=MzI0MDE3MjAzMg==&mid=2648393623&idx=1&sn=e7746da0369977fd88b81d8cfdd707c8&chksm=f1310af8c64683ee772575713aff94b27dcaa80cb2459690ac9946a94b2f9ec2a11c792a8adc&scene=21#wechat_redirect)中介绍的 NioSelectorPool 的read(ByteBuffer buf, NioChannel socket, Selector selector, long readTimeout) 方法。在该方法中又会调用 NioBlockingSelector 的 read() 方法

对于 tomcat 数据读取总结如下：

- 对于请求行，请求头和请求体的读取默认(不开启异步)都在 tomcat io 线程中进行。
- **对于请求行和请求头的读取是非阻塞读取**，即不阻塞 tomcat io 线程，如果没有读取到数据，则由 poll 线程继续监测下次数据的到来。
- **对于请求体的读取是阻塞的读取**，先异步读，如果发现请求体数据不可读，那么首先注册封装的 OP_READ 事件到 BlockPoller 对象实例的事件队列里。然后利用 NioSocketWrapper 对象中的 readLatch 来阻塞 tomcat io 线程。
...
![image-20210510000104403](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20210510000104403.png)
...
- 然后在调用 NioSocketWrapper 中的 CountDownLatch 类型 writeLatch 属性的 await() 方法，使当前线程(一般是tomcat io线程)在 writeLatch 上等待。
...
![image-20210628223926001](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20210628223926001.png)
...
这3个部分，在上面的Java双亲委派模型图中都有体现。不过可以看到ExtClassLoader没有画出来，可以理解为是跟bootstrap合并了，都是去`JAVA_HOME/jre/lib`下面加载类
