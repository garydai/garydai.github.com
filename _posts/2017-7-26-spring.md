...
Java SE（Java Beans、Java ServiceLoader SPI、JNDI）

Java EE（EJB（Enterprise Java Beans）、Servlet）

开源（Apache Avalon、PicoContainer、Google Guice、Srping Framework）
...
```java
<propertiy name="user" ref="userBean">
```
...
依赖查找：bean definition、单例对象

依赖注入：除了上面两个，还有resolved dependency与@Value外部化配置

## 代码结构

beanFactory

![image-20200623092340694](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200623092340694.png)

applicationContext

![image-20200623092523867](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200623092523867.png)
...
7. initializeBean，初始化bean，调用各种aware（BeanNameAware、BeanClassLoaderAware、BeanFactoryAware），完成生命周期初始化调用，applyBeanPostProcessorsBeforeInitialization（BeanPostProcessor#postProcessBeforeInitialization）、invokeInitMethods、applyBeanPostProcessorsAfterInitialization，BeanPostProcessor#postProcessAfterInitialization（aop proxy）

8. destory，DestructionAwareBeanPostProcessor#postProcessBeforeDestruction
...
![image-20200604152341297](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200604152341297.png)
...
this.singletonsCurrentlyInCreation加入a，

AutowiredAnnotationBeanPostProcessor后置处理器发现b属性，

getbean b，

this.singletonsCurrentlyInCreation加入b，

AutowiredAnnotationBeanPostProcessor后置处理器发现a属性，

getbean a，

a在this.singletonsCurrentlyInCreation中，表示a正在创建中，

从singletonFactories工厂生成a，并对a做后置处理器（其中可能有aop代理），
...
protected Object getSingleton(String beanName, boolean allowEarlyReference) {
   ...
   if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
      synchronized (this.singletonObjects) {
         // = null
         // 再从二级缓存里拿未属性注入但已经aop的bean
         singletonObject = this.earlySingletonObjects.get(beanName);
         if (singletonObject == null && allowEarlyReference) {
            // 最后从三级缓存里拿生成二级缓存里的数据的工厂对象，工厂生成对象后放入二级缓存，用singletonFactories工厂生成a
...
```java
		...
		// 如果在populateBean中，当前bean被提早代理的情况下
		// 如果有循环依赖，即有early对象了
		if (earlySingletonExposure) {
      // 返回bean被代理后的对象
			Object earlySingletonReference = getSingleton(beanName, false);
			if (earlySingletonReference != null) {
				if (exposedObject == bean) {
          // 如果当前bean没有被代理（循环依赖的时候已经被代理），则使用之前被代理的对象
					exposedObject = earlySingletonReference;
				}
				else if (!this.allowRawInjectionDespiteWrapping && hasDependentBean(beanName)) {
					String[] dependentBeans = getDependentBeans(beanName);
...
applicationContext组合了beanFactory，本身还提供其他功能，即代理了beanFactory，application获取bean都是通过beanFactory来获取bean
...
配置元信息banDefinition读取与解析、ioc容器生命周期、spring事件发布、国际化等
...
systemProperties、systemEnvironment、ResourcePorperty
...
**InitializingBean** (InitializingBean是每个bean初始化完成后就会调用)

**FactoryBean** 

**SmartInitializingSingleton** (bean的afterSingletonsInstantiated()方法是在所有单例bean都初始化完成后才会调用的。)
