...
- Message是按照topic来组织，每个topic可以分成多个的partition，比如：有5个partition的名为为page_visits的topic的目录结构为：
  ![topic_partition](https://img-blog.csdn.net/20150121163257203)

partition是分段的，每个段叫LogSegment，包括了一个数据文件和一个索引文件，下图是某个partition目录下的文件：
![partition](https://img-blog.csdn.net/20150121163718558)
可以看到，这个partition有4个LogSegment。

![image-20211122205205160](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20211122205205160.png)
...
![image-20210701170451742](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20210701170451742.png)
...
### 每个线程维护一个 KafkaConsumer

![image-20200617162011854](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200617162011854.png)

### 单 KafkaConsumer 实例 + 多 worker 线程



![image-20200617162143949](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200617162143949.png)

消费不保证顺序，引入队列

![image-20200617162402169](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200617162402169.png)
...
是按顺序的，只是周期拉取的，然后放入`**ProcessQueue#**msgTreeMap`等待消费，每次MessageListenerOrderly从takeMessags获取最先生成的消息
...
![image-20200806174018908](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200806174018908.png)
...
![image-20200806083941360](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200806083941360.png)
...
![image-20200806085146288](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200806085146288.png)

### 分区leader选举

ar集合中第一个存活副本，且在isr集合中

当一个broker停止或者crashes时，所有本来将它作为leader的分区将会把leader转移到其它broker上去。这意味着当这个broker重启时，它将不再担任何分区的leader，[kafka](https://so.csdn.net/so/search?q=kafka&spm=1001.2101.3001.7020)的client也不会从这个broker来读取消息，从而导致资源的浪费。比如下面的broker 7是挂掉重启的，我们可以发现Partition 1虽然在broker 7上有数据，但是由于它挂了，所以[Kafka](https://www.iteblog.com/archives/tag/kafka/)重新将broker 3当作该分区的Leader，然而broker 3已经是Partition 6的Leader了。
...
![image-20200806174825895](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20200806174825895.png)
...
![image-20220208170647146](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20220208170647146.png)
...
![image-20220208171725418](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20220208171725418.png)

![image-20220208171741184](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20220208171741184.png)
...
1、首先说明下面的几张图片**来自于互联网共享**，也就是我后面参考文章里面的列出的文章。
...
![image-20210506125824890](https://github.com/garydai/garydai.github.com/raw/master/_posts/pic/image-20210506125824890.png)
